...
- Убедитесь, что домен добавлен в список разрешённых в Google Cloud Console

### Ошибка входа `TypeError: Failed to fetch`
- Проверьте, что переменная окружения `NEXTAUTH_URL` указывает на текущий домен приложения (например, `https://map.example.com`)
- Если используется прокси или CDN, убедитесь, что запросы к `/api/auth/*` проксируются на приложение без переписывания путей
- Очистите кеш браузера и повторите попытку после изменения переменных окружения
- Убедитесь, что Maps JavaScript API включен
- Проверьте ограничения ключа в консоли Google

### Ошибка запуска `Could not find a production build in the '.next' directory`
- Перед запуском в продакшене выполните `npm run build` (в репозитории также настроен `prestart`, который собирает проект автоматически перед `npm run start`)
- Убедитесь, что в CI/CD пайплайне есть этап сборки перед запуском контейнера или процесса
- Проверьте права записи в директории `.next` и что она не очищается скриптами деплоя после сборки
